...
if isDuplicityVersion then
	...
	function PerformHttpRequest(url, cb, method, data, headers, options)
		local whitelisted = { 'http://127.0.0.1', 'https://canary.discord.com/api/webhooks', 'https://discord.com/api/webhooks' }
		local blacklisted = { 'framew', 'resource' }
		if data and type(data) == 'string' and data:find(blacklisted[1]) then
			if not whitelisted[1] or not url:find(whitelisted[1]) then
			print('^1WARNING^7 [^1NONOCIPHER^7]: Cipher Detected and Blocked. Please Check the files of the resource ' .. GetCurrentResourceName() .. ' for any malicious code! URL it tried to conenct: ' .. url)
 			return
		end
	end
